...
public class UserMessageDefaultService implements UserMessageService {
    ...
    protected Map<String, InputStream> getMessageContentWithAttachments(String messageId) throws MessageNotFoundException {
        ...
        RawEnvelopeDto  rawEnvelopeDto = rawEnvelopeLogDao.findRawXmlByMessageId(messageId);
        Map<String, InputStream> result = new HashMap<>();
        UserMessage userMessage = getUserMessageById(messageId);
        InputStream messageStream = messageToStream(userMessage);
        if (rawEnvelopeDto!= null && rawEnvelopeDto.getRawMessage()!=null) {
            result.put("soap-envelope.xml", new ByteArrayInputStream(rawEnvelopeDto.getRawMessage().getBytes()));

            try {
                InputStream messageData = getTimestampData(rawEnvelopeDto.getRawMessage(), messageId);
                result.put("timestamp-result.txt", messageData);
    ...
    public InputStream  getTimestampData(String soapEnvelope, String messageId) throws IOException {
        // get data from EBSI
        ...
        try (StringReader stringReader = new StringReader(soapEnvelope); InputStream targetStream =
            ...
            if (result!=null) {
                stringWriter.write("Message id: ");
                stringWriter.write(messageId);
                stringWriter.write("\n");


                stringWriter.write("NotarizedHashValue: ");
                stringWriter.write(result.getStringValue("NotarizedHashValue"));
                stringWriter.write("\n");

                stringWriter.write("SignatureHashValue: ");
                stringWriter.write(result.getStringValue("SignatureHashValue"));
                stringWriter.write("\n");

                LocalDateTime time = result.getLocalDateTimeValue("Timestamp");
                stringWriter.write("Timestamp: ");
                stringWriter.write(time.toString());
                stringWriter.write("\n");

                stringWriter.write("RegisteredBy: ");
                stringWriter.write(result.getStringValue("RegisteredBy"));
                stringWriter.write("\n");

                stringWriter.write("TxHash: ");
                stringWriter.write(result.getStringValue("TxHash"));
                stringWriter.write("\n");

                stringWriter.write("ValidationUrl: ");
                stringWriter.write(result.getStringValue("ValidationUrl"));
                stringWriter.write("\n");
            }
