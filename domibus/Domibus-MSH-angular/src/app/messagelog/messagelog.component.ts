...
import {MessageTimestampDetailsComponent} from 'app/messagelog/messagetimestamp-details/messagetimestamp-details.component';
...
import {MessageTimestampEntry} from "./support/messagetimestampentry";
...
export class MessageLogComponent extends mix(BaseListComponent)
  ...
  static readonly VALIDATE_TIMESTAMP_MESSAGE_URL: string = 'rest/timestamp/validate?messageId=${messageId}';
  static readonly DOWNLOAD_TIMESTAMP_MESSAGE_URL: string = 'rest/timestamp/download?messageId=${messageId}';
  ...
  @ViewChild('timestampAction', {static: false}) timestampAction: TemplateRef<any>;
  ...
  private configureColumnPicker() {
    ...
    this.columnPicker.allColumns.push(
      ...
      {
        cellTemplate: this.timestampAction,
        name: 'EBSI Timestamp',
        width: 80,
        sortable: false
      }
    ...
    this.columnPicker.selectedColumns = this.columnPicker.allColumns.filter(col => {
      return ['Message Id', 'From Party Id', 'To Party Id', 'Message Status', 'Received', 'AP Role', 'Message Type', 'Actions', 'EBSI Timestamp'].indexOf(col.name) != -1
    });
  }
  ...
  isTimestampButtonEnabledAction(row): boolean {
    return this.isRowTimestampButtonEnabled(row);
  }
  ...
  private isRowTimestampButtonEnabled(row): boolean {
    return !this.isSplitAndJoinMessage(row);
  }
  ...
  private async downloadSignature(row) {
    const messageId = row.messageId;
    try {
        const downloadUrl = MessageLogComponent.DOWNLOAD_TIMESTAMP_MESSAGE_URL.replace('${messageId}', encodeURIComponent(messageId));
        DownloadService.downloadNative(downloadUrl);
    } catch (err) {
      this.alertService.exception(`Could not download message content for id ${messageId}.`, err);
    }
  ...
  private async validateMessageTimestamp(row) {
    const messageId = row.messageId;
    try {
        const validateUrl = MessageLogComponent.VALIDATE_TIMESTAMP_MESSAGE_URL.replace('${messageId}', encodeURIComponent(messageId));
        const messageTimestampEntry = await this.http.get<MessageTimestampEntry>(validateUrl).toPromise();
        this.showTimestampDetails(messageTimestampEntry);
    } catch (err) {
      this.alertService.exception(`Could not validate timestamp message content for id ${messageId}.`, err);
    }
  }
  ...
  validateTimestampAction(row) {
    this.validateMessageTimestamp(row);
  }

  downloadSignatureAction(row) {
    this.downloadSignature(row);
  }
  ...
  showTimestampDetails(selectedTimestampInfo: any) {
    this.dialog.open(MessageTimestampDetailsComponent, {
      data: {timestampInfo: selectedTimestampInfo}
    });
  }
